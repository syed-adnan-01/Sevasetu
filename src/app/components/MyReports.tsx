...
export function MyReports() {
  ...
  const fetchReports = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/reports/user/${user?.email}`);
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-400">View and track the status of your submitted issues</p>
          </div>
          <button 
                        ...
                        <div className="flex items-center justify-between border-t border-gray-700/50 pt-4 mt-auto">
                          <div className="flex items-center gap-4">
                            <div className="flex items-center gap-1 text-gray-400 text-xs bg-gray-900/50 px-2 py-1 rounded-lg border border-gray-800">
                              ...
                              <span>{report.location?.address || addresses[report._id] || "Location Verified"}</span>
                            </div>
                            <div className="flex items-center gap-1 text-gray-400 text-xs bg-gray-900/50 px-2 py-1 rounded-lg border border-gray-800 capitalize">
                              <span className={`w-2 h-2 rounded-full ${report.status === 'resolved' ? 'bg-green-500' : 'bg-yellow-500'}`}></span>
                              {report.status}
                            </div>
                          </div>
